<h2>Reject Agen <?php echo $nama_agen?></h2>
...
<div class="yellowbox">
    ...
    <div id="agen">
        <select name="agen">
            ...
            ?>
            <option value="<?php echo $agen['id']?>" <?php if ($this->session->userdata('id_agen') == $agen['id']) echo 'selected="selected"'; ?> onclick="window.location='index.php/reject_agen/pilih_agen/<?php echo $agen['id']?>'"><?php echo $agen['kode']?>, <?php echo $agen['nama']?></option>
            <?php }?>
        </select>
...
<div style="text-align: right">
    <input type="button" value="Refund" class="button blue" onclick="location.href = 'index.php/reject_agen/refund'" <?php if (count($this->cart->contents()) == 0) echo 'disabled="disabled"'; ?>/>
...
function load_model(id){
    $('#model').load('index.php/reject_agen/model/' + id);
    $('#harga').val("");
}

function load_warna(model){
    $('#warna').load('index.php/reject_agen/warna/' + model);
    $('#harga').val("");
}

function load_ukuran(model,warna){
    $('#ukuran').load('index.php/reject_agen/ukuran/' + model + '/' + warna);
    $('#harga').val("");
}

function load_harga(id_produk){
    $.get('index.php/reject_agen/harga/' + parseInt(id_produk), function(harga){
        $('#harga').val(harga);
    });
}
