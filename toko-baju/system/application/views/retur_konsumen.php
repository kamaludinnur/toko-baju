...
<div style="text-align: right">
    <input type="button" value="Refund" class="button blue" onclick="location.href = 'index.php/retur_konsumen/refund'" <?php if (count($this->cart->contents()) == 0) echo 'disabled="disabled"'; ?>/>
    <input type="button" value="Batal" class="button red"  onclick="if(confirm('Yakin akan membatalkan? Transaksi yang belum dibayar akan terhapus.')) location.href = 'index.php/retur_konsumen/batal'"/>
</div>
...
function load_model(id){
    $('#model').load('index.php/retur_konsumen/model/' + id);
    $('#harga').val("");
}

function load_warna(model){
    $('#warna').load('index.php/retur_konsumen/warna/' + model);
    $('#harga').val("");
}

function load_ukuran(model,warna){
    $('#ukuran').load('index.php/retur_konsumen/ukuran/' + model + '/' + warna);
    $('#harga').val("");
}

function load_harga(id_produk){
    $.get('index.php/retur_konsumen/harga/' + parseInt(id_produk), function(harga){
        $('#harga').val(harga);
    });
}
