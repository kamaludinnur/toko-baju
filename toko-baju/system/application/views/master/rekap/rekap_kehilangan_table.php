<?php $total_jumlah = 0; $total_harga = 0; $total_kerugian = 0 ?>

<?php if(count($data_rekapan) > 0) : ?>
<form action="index.php/master/rekap/kehilangan_xls" method="post" style="float: right; margin-bottom: 10px">
...
<table class="rounded-corner sortable" id="main_table">
    <thead>
        <tr>
            ...
            <th>Jumlah</th>
            <th>Harga</th>
            <th class="rounded-q4">Kerugian</th>
        </tr>
    </thead>
    <tbody>
    <?php if (count($data_rekapan) == 0) : ?>
    ...
    <?php else : $i = 1; foreach($data_rekapan as $data) : ?>
        <tr>
            ...
            <td style="text-align: right"><?php echo number_format($data['kerugian'], 0, ',', '.'); ?></td>
        </tr>
    <?php
    $total_jumlah += $data['jumlah'];
    $total_harga  += $data['harga'];
    $total_kerugian += $data['kerugian'];
    $i++; endforeach; endif; ?>
    </tbody>
    <tfoot>
        <tr style="font-size: 15px;">
            <td colspan="8" style="text-align: right"><strong>Total kerugian</strong></td>
            <td style="text-align: right"><?php echo number_format($total_kerugian, 0, ',', '.'); ?></td>
        </tr>
    </tfoot>
</table>
